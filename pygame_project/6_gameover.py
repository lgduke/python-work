...
# 게임 종료 메시지(Time out(시간초과), Mission complated(성공), Game over(캐릭터 공에 맞음 실패) )
game_result = "Game Over! Good Luck"
...
# 2. 이벤트 처리(키보드 마우스)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({ 
                        ...
                        "img_idx" : ball_img_idx + 1 , # 어느 공의 좌표 인지.. 0은 제일 큰공
                        "to_x" : -3 , # 공의 X축 이동방향 -3이면 왼쪽 +3이면 오른쪽
                        "to_y" : -6 ,# Y축 이동방향
            ...
            continue #안쪽 for문이 조건이 맞지않으면 continue, 바깥 for문 계속 수행
        break # 안쪽 for문에서 break를 만나 여기로 진입가능 2중 for문을 한번에 탍출
